...
def ensure_db():
    # /tmp Render Free’da yazılabilir
...
def on_startup():
    ensure_db()
    # admin’i bir kez tohumla; tekrar eklemeye çalışmaz
...
# Küresel koruma: login olmadan /login, /health, /static harici hiçbir yere girilemez
...
class AuthWall(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if not u:
            # sadece giriş sayfasına yönlendir
            nxt = path
...
def index(request: Request):
    u = user_in_session(request)  # AuthWall sayesinde zaten var
...
async def create_plan(
    ...
    u = user_in_session(request)  # AuthWall -> garanti
    require_csrf(request, csrf_token)
